              ...
              <div className="space-y-4">
                ...
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {(shoppingOffers || []).map((offer, index) => {
                    const hasLink = Boolean(offer.link && offer.link !== '#');

                    if (!hasLink) {
                      return (
                        <div
                          key={`no-link-${index}`}
                          className="rounded-2xl border border-gray-100 bg-gray-50 p-4 opacity-75"
                        >
                          <p className="line-clamp-2 text-sm font-semibold text-gray-500">
                            {offer.produto}
                          </p>
                          <p className="mt-1 text-sm text-gray-400">{offer.loja}</p>
